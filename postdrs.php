...
<?php
	...
	$PostDrsContentList = $wordpress_posts->getPostContentListByPost_Type('postdr');
	
	// print_r($PostDrsContentList);
	...
	print "<section id=\"professor_list\" class=\"container\">\n";
		print "\t<div class=\"center\">\n";
			print "\t\t<div class=\"col-xs-12 col-sm-12 blog-content\">\n";
				print "\t\t\t<h3 style=\"text-align:left;\">".$page_title."</h3>\n";
				print "\t\t\t<table>\n";
	if ( sizeof($PostDrsContentList) >= 1 ) {
		$counter = 1;
					print "\t\t\t\t<tr>\n";
						print "\t\t\t\t\t<th>姓名</th>\n";
						...
						print "\t\t\t\t\t<th>研究專長</th>\n";
					print "\t\t\t\t</tr>\n";
		foreach ($PostDrsContentList as $key => $value) {
			$PostMeta_List = $wordpress_posts->getPostMeta_List($value['ID']);
			if ( sizeof($PostMeta_List) >= 1 ) {
				...
				foreach ($PostMeta_List as $key3 => $value3) {
					...
					print "\t\t\t\t</tr>\n";
						print "\t\t\t\t\t<td><a href=\"postdrs_detail.php?i=".base64_encode($value['ID'])."\">".$name."</a></td>\n"; // print "\t\t\t\t\t<td><a href=\"postdrs_detail.php?i=".base64_encode($value['ID'])."\">".$value['post_title']."</a></td>\n";
						print "\t\t\t\t\t<td>".$supervisor."</td>\n";
						print "\t\t\t\t\t<td>".$room."</td>\n";
						print "\t\t\t\t\t<td>".$email."</td>\n";
						print "\t\t\t\t\t<td>".$researches ."</td>\n";
					print "\t\t\t\t</tr>\n";
			}
		}
	}
