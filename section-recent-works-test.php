...
<?php
	...
	foreach ($content_array as $key => $value) {
		...
		*/
		...
		// 161122-1448
		// REF: http://blog.oskoui-oskoui.com/?p=3234
		// 
		$myImage = '';
		//$myImage = imagecreatefromjpeg($img_address);
		// print_r(getimagesize($img_address));
		// Array ( [0] => 512 [1] => 288 [2] => 3 [3] => width="512" height="288" [bits] => 8 [mime] => image/png )
		$image_info_array = getimagesize($img_address);
		// print $image_info_array['mime'];
		if ( strpos($image_info_array['mime'], 'jpeg') !== false ) {
			// print 'jpeg';
			$myImage = imagecreatefromjpeg($img_address);
		} else if ( strpos($image_info_array['mime'], 'jpg') !== false ) {
			// print 'jpg';
			$myImage = imagecreatefromjpeg($img_address);
		} else if ( strpos($image_info_array['mime'], 'gif') !== false ) {
			// print 'gif';
			$myImage = imagecreatefromgif($img_address);
		} else if ( strpos($image_info_array['mime'], 'png') !== false ) {
			// print 'png';
			$myImage = imagecreatefrompng($img_address);
		}
		$myImageCrop = imagecreatetruecolor(600, 455);
		$b = imagecopyresampled($myImageCrop,$myImage,0,0,0,0,600,455,$image_info_array[0],$image_info_array[1]);
		if ($b == 0 ) {
			print $img_address; // print $b;
		}
		imagejpeg($myImageCrop, $counter.'.jpg');
		/*
		$myImageCrop = imagecreatetruecolor(600, 455);
		$b = imagecopyresampled($myImageCrop,$myImage,0,0,0,0,600,455,$image_info_array[0],$image_info_array[1]);
		...
		$a = imagejpeg($myImageCrop, null, 100);
		imagedestroy($myImageCrop);
		*/
		...
		print "<img class=\"img-responsive\" src=\"".$counter.'.jpg'."\" alt=\"\">\n";
